...
def _get_inception_layer(sess):
    ...
    layer_name = 'FID_Inception_Net/pool_3:0'
...
def get_activations(images, sess, batch_size=50, verbose=False):
    ...
    n_used_images = n_batches * batch_size
    predict_arr = np.empty((n_used_images, 2048))
    for i in range(n_batches):
        ...
        predict = sess.run(inception_layer, {'FID_Inception_Net/ExpandDims:0': batch})
        predict_arr[start:end] = predict.reshape(batch_size, -1)
    if verbose:
        print(" done")
    return predict_arr
...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    -- mu2   : The sample mean over activations of the pool_3 layer, pre-calculated
               on an representive data set.
    -- sigma1: The covariance matrix over activations of the pool_3 layer for
               generated samples.
    -- sigma2: The covariance matrix over activations of the pool_3 layer,
               pre-calculated on an representive data set.
    ...
    return diff.dot(diff) + np.trace(sigma1) + np.trace(sigma2) - 2 * tr_cov_mean
...
def calculate_fid_given_paths(paths, inception_path, low_profile=False):
    ...
    with tf.Session() as sess:
        ...
        calc_fid_value = calculate_frechet_distance(m1, s1, m2, s2)
        return calc_fid_value
